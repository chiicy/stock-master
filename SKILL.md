---
name: stock-master
description: 自用股票总控 Skill。面向 A 股单股、板块、指数和市场分析，统一调度技术面、基本面、资金面、情绪面、板块面、消息面、预判面等 reference，并吸收 `stock-analysis`、`stock-analysis-xiaping-remix`、`value-investing-detective`、`akshare-stock` 的方法论。适用于“分析某股票”“全面分析某股票”“多角度分析”“技术面/基本面/资金面怎么看”“板块怎么看”“综合判断后市”等请求。主 skill 只负责路由、选视角、组织输出；统一数据获取走 `datasource` 包。
---
...
- 来自 `stock-analysis`
  - 标准技术指标口径：RSI / MACD / KDJ / 布林带 / 均线结构
  - 技术信号阈值与市场环境联读
  - 个股分析要结合大盘、板块、北向和资金面
- 来自 `stock-analysis-xiaping-remix`
  - 单股深度技术面写法：支撑位 / 压力位 / 缺口观察 / 未来 3 个交易日判断
  - 明确区分“工具直接返回的事实”和“基于指标的推断”
  - 未来 3 日只做条件化方向判断，不做伪精确预测
- 来自 `value-investing-detective`
  - 证据优先、可复核推理、三表联动、杜邦拆解、ROIC/WACC、DCF、护城河、造假风险、同业对比
  - 深度价投报告骨架 + 1→22 侦探式清单
  - 公式口径统一与法证 / ESG 红旗模板
  - 深度基本面分析必须先做数据完整性检查，不满足条件时要降级为轻量判断
  - 每个结论尽量写成“因为……（数据/事实）所以……”的证据链
- 来自 `akshare-stock`
  - Router -> Service/DataSource -> Analyzer -> Formatter 的分层思路
  - 任务类型优先于标的解析的路由方式
  - 数据源 fallback、缓存、紧凑输出和视角化拆分
...
- `src/stock_master/datasource/providers/*.py`：每个数据源一个文件，负责各层 provider 实现
...
3. 判断是轻量分析还是深度分析：
   - 深度技术面：用户明确要支撑位 / 压力位 / 缺口 / 未来三天 / 像虾评一样
   - 深度基本面：用户明确要财报侦探 / 估值 / DCF / ROIC / 杜邦，且数据较完整
...
新增数据源时：
- 优先在 `scripts/datasource/providers/<source>.py` 中新增 provider，而不是继续把逻辑堆进单文件
- 一个数据源一个文件，便于后续独立扩充和测试
- provider 只重写自己支持的方法；不支持、报错、超时都返回 `False`
- 由 `scripts/datasource/runtime.py` 按优先级统一 fallback
...
| `get_news` | `aggregate` | A 股优先 `opencli-sinafinance + opencli-xueqiu + opencli-bloomberg`；全球优先 `opencli-bloomberg + opencli-yahoo-finance + opencli-xueqiu` | 对外返回 `source="merged"` |
...
- `scripts/stock_analysis/*` 应直接依赖 `datasource` 包和内部模块，不要反向依赖 facade 脚本
- `scripts/stock_common/*` 承担共享工具职责，避免再长出新的 `utils.py`
